...
class ORM_Core {
	...
	/** @var string  Table primary value name */
	protected $primary_val = 'name';

	/** @var array Array of foreign key name overloads */
	 ...
	 * @var Indicator of using transaction
	 */
	protected $is_in_transaction = false;

	/**
	 * @author Ondřej Fibich
	 * @var array[boolean]  Indicator of using logger logic
	 */
	protected $is_logger_on = array
	(
		/* Logger enabling on add */
		'add'		=> true,
		/* Logger enabling on update */
		'update'	=> true,
		/* Logger enabling on delete */
		'delete'	=> true
	);
	 ...
	 * @param   string  order by key [if not set ordered by value]
	 * @return  array
	 */
	public function select_list($key = 'id', $val = 'name', $order_val = FALSE)
	{
		if ($key === NULL)
		{
	...
	public function save()
	{
		if ( ! empty($this->changed))
		{
			...
			if ($this->loaded === TRUE)
			{
				...
				// log update action
				if (self::is_logger_enabled() && $this->is_logger_on['update'])
				{
					$this->log_action(
						Log_Model::ACTION_UPDATE,
			...
			else
			{
				...
				if ($query->count() > 0)
				{
					...
					// log add action
					if (self::is_logger_enabled() && $this->is_logger_on['add'])
					{
						$this->log_action(
							Log_Model::ACTION_ADD,
							$this->object[$this->primary_key],
							$data
						);
					}
	...
	public function delete($id = NULL)
	{
		...
		if (self::is_logger_enabled() && $this->is_logger_on['delete'] && count($result) == 1)
		{
	 ...
	 * @throws Exception  If can't open transaction
	 */
	 ...
	 * @throws Exception  If can't commit transaction
	 */
	 ...
	 * @throws ErrorException  If can't rollback transaction
	 */
	 ...
	 * @throws  Kohana_Database_Exception  If save was not successful.
	 */
	public function delete_throwable($id = NULL)
	{
		if ($id === NULL AND $this->loaded)
		{
			// Use the the primary key value
		...
		// add log for deleting
		if (self::is_logger_enabled() && $this->is_logger_on['delete'])
		{
			$this->log_action(Log_Model::ACTION_DELETE, $id);
		}
	 ...
	 * @author Ondřej Fibich
	 * @param string	$action	[optional] Action name (add, delete, update)
	 *				or NULL for all action fields
	 * @return bool
	 */
	public function is_logger_on($action = NULL)
	{
		if (!empty($action) && array_key_exists($action, $this->is_logger_on))
		{
			return $this->is_logger_on[$action];
		}
		else
		{
			return	$this->is_logger_on['add'] &&
				$this->is_logger_on['update'] &&
				$this->is_logger_on['delete'];
		}
	}

	/**
	 * Set status of single action logger to property is_on if action is filled in.
	 * If action is not filled, all fields are set.
	 * @author Ondřej Fibich
	 * @param bool		$is_on	Inicator of new state
	 * @param string	$action	[optional] Key of action field [add, delete, update]
	 *				or NULL for all action fields
	 */
	public function set_logger($is_on, $action = NULL)
	{
		if (!empty($action) && array_key_exists($action, $this->is_logger_on))
		{
			$this->is_logger_on[$action] = ($is_on === TRUE);
		}
		else
		{
			$this->is_logger_on['add']	= ($is_on === TRUE);
			$this->is_logger_on['update']	= ($is_on === TRUE);
			$this->is_logger_on['delete']	= ($is_on === TRUE);
		}
	}

	/**
	 * Add action to logger
	 * @author Ondřej Fibich
	 * @param int $action  Action type
	 * @param int $id      ID of object who is manipulated in action
	 * @param array $data  Changed data in array or null
	 */
	protected function log_action($action, $id, $data = NULL)
	{
		$id = $id ? intval($id) : 0;
		$who = isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : 0;
		$values = empty($data) ? NULL : json_encode($data);

		if ($id && $who && $action >= Log_Model::ACTION_ADD &&
		    $action <= Log_Model::ACTION_UPDATE)
		{
			$log = array
			(
	...
	/**
	 * Get MySQL version comparable with function <code>version_compare</code>
	 * @author Ondřej Fibich
	 * @staticvar string $version	Cache for version
	 * @return string				Version in shape d.d.d
	 */
	public function get_mysql_version()
	{
		...
		if ($version == null)
		{
			$ver = $this->db->query("SELECT VERSION() as version")->current()->version;
			if (mb_eregi('^([0-9]+\.[0-9]+\.[0-9]+)', trim($ver), $r))
			{
				$version = $r[1];
			}
			else
			{
				throw new Kohana_Exception("Cannot get MySQL version.");
			}
		}
	...
	/**
	 * Check if action logging is enabled on server
	 * @author Ondřej Fibich
	 * @staticvar string $logger_on		Cache
	 * @return							true if is enabled otherwise false
	 */
	protected static function is_logger_enabled()
	{
		static $logger_on = null;

		if ($logger_on == null)
		{
			$logger_on = Settings::get('action_logs_active') == 1;
		}
	...
	/**
	 * Check if MySQL permission for current user
	 *
	 * @author Michal Kliment
	 * @staticvar string $permissions
	 ...
	 *
	 * @staticvar array $tables	Cache
	 * @param string $table		Table name
	 * @return boolean			true if table is founded, false otherwise
	 */
	...
	{
		return array_key_exists($property, $this->object);
	}
	 ...
	 * 
	 * @author Ondřej Fibich
	 * @staticvar array $cache		Cache for columns
	 * @param type $column_name		Name of searched column
	 * @return boolean				
	 */
